...
After the pmo-entry block, decide who the entry's natural owner is. If that owner is clearly a DIFFERENT role from the one raising it (for example: a client-driven scope change to be priced -> commercial, an engineering rework trend -> engineering_manager, a procurement-driven change -> procurement), ALSO append ONE ```actions block as the very last thing — AFTER the pmo-entry block — containing exactly one suggested task for that role: the concrete next step on the entry just raised (assess it, propose the response, quantify the impact). Use "source_ref": null — the entry only receives its register ID when the user saves it — and name the entry by its short description inside the task text. The app renders this as a separate "Assign actions" confirm card that the user can confirm or ignore, so it is a suggestion, never an automatic assignment. If the raising role is the natural owner, or no clear owner emerges, do NOT append the block.
