  ...
  return (
    <div className="container mx-auto max-w-screen-lg px-6 py-8">
      ...
      <p className="mt-2 max-w-3xl text-[15px] leading-relaxed text-muted-foreground">
        ...
        companion to the business-friendly <Link href={`/concepts`} className="font-medium text-foreground hover:underline">Concepts deck</Link>.
      </p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {SECTIONS.map((s) => {
          ...
          return s.href ? (
            <Link key={s.title} href={`/technical/${s.href}`} className="rounded-xl border bg-card p-5 transition hover:-translate-y-0.5 hover:border-foreground/20 hover:shadow-sm">
              {inner}
            </Link>
          ) : (
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        ...
        <Link href={`/dashboard`} className="rounded-md border px-4 py-2 text-sm font-medium transition hover:bg-muted">Back to portfolio</Link>
      </div>
    </div>
  );
}
