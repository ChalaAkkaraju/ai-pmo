      ...
      {TIERS.map((t) => (
        <section key={t.tier} className="mt-7">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {t.items.map((it) => (
              <Link key={it.href} href={`/access/${token}/${it.href}`} className="group rounded-xl border bg-card p-4 transition hover:-translate-y-0.5 hover:border-foreground/20 hover:shadow-sm">
