/**
 * Sign-in page. Invite-only: there is no sign-up link — accounts are seeded by
 * the AI PMO owner (scripts/seed-users.ts). On success the server action
 * redirects to `next` (default /account).
 */
  ...
  return (
    <main className="container mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-20">
      ...
      <p className="mt-1.5 text-sm text-muted-foreground">
        Use the email and password provided by your AI PMO owner.
      </p>
      ...
      <form action={signIn} className="mt-6 space-y-4">
        ...
        <button
          type="submit"
          className="w-full rounded-md bg-foreground px-3 py-2 text-sm font-medium text-background transition hover:opacity-90"
        >
          Sign in
        </button>
      </form>
