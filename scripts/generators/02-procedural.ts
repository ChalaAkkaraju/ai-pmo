...
function fillPlaceholders(template: string, r: () => number): string {
  return template
    ...
    // Numeric durations / counts — placeholders FOLLOWED by their unit word must resolve to numbers.
    .replace(/\{months?\}/g, () => String(intInRange(2, 10, r)))
    .replace(/\{weeks\}/g, () => String(intInRange(3, 16, r)))
    .replace(/\{days\}/g, () => String(intInRange(3, 30, r)))
    .replace(/\{n_days\}/g, () => String(intInRange(2, 14, r)))
    .replace(/\{n_turbines\}/g, () => String(intInRange(20, 120, r)))
    .replace(/\{n_workers\}/g, () => String(intInRange(40, 400, r)))
    .replace(/\{n_joints\}/g, () => String(intInRange(200, 6000, r)))
    .replace(/\{n_crossings\}/g, () => String(intInRange(2, 18, r)))
    .replace(/\{n_strings\}/g, () => String(intInRange(20, 200, r)))
    .replace(/\{n_boxes\}/g, () => String(intInRange(10, 200, r)))
    .replace(/\{n_circuits\}/g, () => String(intInRange(2, 12, r)))
    .replace(/\{n_loops\}/g, () => String(intInRange(2, 20, r)))
    .replace(/\{n_pallets\}/g, () => String(intInRange(50, 2000, r)))
    .replace(/\{n_anchors\}/g, () => String(intInRange(10, 200, r)))
    .replace(/\{local_n\}/g, () => String(intInRange(2, 12, r)))
    .replace(/\{miles\}/g, () => String(intInRange(2, 80, r)))
    .replace(/\{pct\}/g, () => String(intInRange(3, 30, r)))
    .replace(/\{delta\}/g, () => String(intInRange(2, 15, r)))
    .replace(/\{itc_m\}/g, () => String(intInRange(5, 80, r)))
    .replace(/\{ptc_value\}/g, () => pick(['$26/MWh', '$27.50/MWh', '2.75 cents/kWh'], r))
    .replace(/\{wind_threshold\}/g, () => String(intInRange(20, 28, r)))
    .replace(/\{block_id\}/g, () => `B-${String(intInRange(1, 12, r))}`)
    .replace(/\{row_id\}/g, () => `R-${String(intInRange(1, 40, r))}`)
    .replace(/\{mp\}/g, () => String(intInRange(50, 800, r)))
    .replace(/\{k\}/g, () => String(intInRange(2, 20, r)))
    .replace(/\{m\}/g, () => String(intInRange(2, 50, r)))
    .replace(/\{ids\}/g, () => String(intInRange(2, 8, r)))
    .replace(/\{original_module\}/g, () => pick(['the original module', 'the baseline module'], r))
    // Categorical
    .replace(/\{region\}/g, () => pick(['the Midwest', 'the Gulf Coast', 'the Southwest', 'the Northeast', 'the Pacific Northwest'], r))
    .replace(/\{season\}/g, () => pick(['winter', 'the wet season', 'the monsoon season', 'Q1'], r))
    .replace(/\{quarter\}/g, () => pick(['Q1', 'Q2', 'Q3', 'Q4'], r))
    .replace(/\{port\}/g, () => pick(['the port of entry', 'the discharge port', 'the marshalling port'], r))
    .replace(/\{tech_pkg\}/g, () => pick(['the process package', 'the licensor package', 'the technology package'], r))
    .replace(/\{utility_type\}/g, () => pick(['the interconnecting utility', 'the host utility', 'the transmission operator'], r))
